...
# Web Interface Guidelines (Full Reference)
...
- **Design forgiving interactions.** Controls minimize finickiness with generous hit targets, clear affordances, & predictable interactions.
...
- **Clean drag interactions.** Disable text selection & apply `inert` while an element is dragged so selection/hover don't occur simultaneously.
...
- **Non-breaking spaces for glued terms.** Use a non-breaking space `&nbsp;` to keep units, shortcuts & names together: `10 MB` → `10&nbsp;MB`.
...
- **Don't block typing.** Even if a field only accepts numbers, allow any input & show validation feedback.
...
- **Text replacements & expansions.** Some input methods add trailing whitespace. The input should trim the value.
...
- **Don't use the main thread for expensive work.** Move especially long tasks to Web Workers.
...
- **Set the appropriate color-scheme.** Style the `<html>` tag with `color-scheme: dark` in dark themes.
- **Text anti-aliasing & transforms.** Scaling text can change smoothing. Prefer animating a wrapper instead of the text node.
- **Avoid gradient banding.** Fading content to dark colors using css masks can cause banding.
...
- **Consistent currency formatting.** Display currency with either 0 or 2 decimal places, never mix.
- **Separate numbers & units with a space.** Instead of `10MB` say `10 MB`.
...
- **Avoid ambiguity.** Labels are clear & specific. Instead of "Continue" say "Save API Key".
